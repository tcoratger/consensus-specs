...
def run_attester_slashing_processing(spec, state, attester_slashing, valid=True):
    ...
    pre_slashing_balances = {
        slashed_index: get_balance(state, slashed_index) for slashed_index in slashed_indices
    }
...
# Note(jtraglia): On an M1 MacBook Air, a test under the minimal preset with the maximum
