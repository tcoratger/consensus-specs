...
def build_deposit_data(
    spec, pubkey, privkey, amount, withdrawal_credentials, fork_version=None, signed=False
):
...
def prepare_pending_deposit(
    ...
    deposit_data = build_deposit_data(
        spec, pubkey, privkey, amount, withdrawal_credentials, fork_version, signed
    )
...
def run_deposit_processing_with_specific_fork_version(
    ...
    yield from run_deposit_processing(
        spec, state, deposit, validator_index, valid=valid, effective=effective
    )
