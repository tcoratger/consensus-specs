...
def test_invalid_one_expected_full_withdrawal_and_none_in_withdrawals(spec, state):
    ...
    next_slot(spec, state)
    execution_payload = build_empty_execution_payload(spec, state)
    execution_payload.withdrawals = spec.Withdrawals()
    execution_payload.block_hash = compute_el_block_hash(spec, execution_payload, state)
...
def test_invalid_max_per_slot_full_withdrawals_and_one_less_in_withdrawals(spec, state):
    prepare_expected_withdrawals(
        spec, state, rng=random.Random(42), num_full_withdrawals=spec.MAX_WITHDRAWALS_PER_PAYLOAD
    )

    next_slot(spec, state)
    execution_payload = build_empty_execution_payload(spec, state)
    execution_payload.withdrawals = spec.Withdrawals(data=execution_payload.withdrawals[:-1])
    execution_payload.block_hash = compute_el_block_hash(spec, execution_payload, state)
...
def test_invalid_max_per_slot_partial_withdrawals_and_one_less_in_withdrawals(spec, state):
    prepare_expected_withdrawals(
        spec, state, rng=random.Random(42), num_partial_withdrawals=spec.MAX_WITHDRAWALS_PER_PAYLOAD
    )
...
def test_invalid_incorrect_withdrawal_index(spec, state):
    ...
    next_slot(spec, state)
    execution_payload = build_empty_execution_payload(spec, state)
    execution_payload.withdrawals[0].index += spec.WithdrawalIndex(1)
    execution_payload.block_hash = compute_el_block_hash(spec, execution_payload, state)
...
def test_invalid_incorrect_amount_full(spec, state):
    ...
    next_slot(spec, state)
    execution_payload = build_empty_execution_payload(spec, state)
    execution_payload.withdrawals[0].amount += spec.Gwei(1)
    execution_payload.block_hash = compute_el_block_hash(spec, execution_payload, state)
...
def test_invalid_one_of_many_incorrectly_full(spec, state):
    ...
    withdrawal.index += spec.WithdrawalIndex(1)
    withdrawal.address = spec.ExecutionAddress(b"\x99" * 20)
    withdrawal.amount += spec.Gwei(4000000)
    execution_payload.block_hash = compute_el_block_hash(spec, execution_payload, state)
...
def test_withdrawable_epoch_but_0_effective_balance_nonzero_balance(spec, state):
    ...
    yield from run_withdrawals_processing(
        spec, state, execution_payload, num_expected_withdrawals=1
    )
...
def test_no_withdrawals_but_some_next_epoch(spec, state):
    ...
    for index in range(3):
        set_validator_fully_withdrawable(spec, state, index, current_epoch + spec.Epoch(1))
        ...
        if rng.choice([True, False]):
            ...
            if rng.randint(0, 7) == 0:
                validator.withdrawal_credentials = (
                    spec.BLS_WITHDRAWAL_PREFIX + validator.withdrawal_credentials[1:]
                )
            ...
            if rng.randint(0, 7) == 0:
                validator.withdrawable_epoch += spec.Epoch(1)
...
def test_success_excess_balance_but_no_max_effective_balance(spec, state):
    ...
    validator.effective_balance = spec.MAX_EFFECTIVE_BALANCE - spec.Gwei(1)
...
def test_success_one_partial_withdrawable_in_exit_queue(spec, state):
    ...
    assert not spec.is_active_validator(
        state.validators[validator_index], spec.get_current_epoch(state) + spec.Epoch(1)
    )
...
def test_success_two_partial_withdrawable(spec, state):
    ...
    yield from run_withdrawals_processing(
        spec, state, execution_payload, num_expected_withdrawals=2
    )
...
def test_success_max_plus_one_withdrawable(spec, state):
    # Sanity check that this test works for this state
    assert len(state.validators) >= int(spec.MAX_WITHDRAWALS_PER_PAYLOAD) + 1
...
def run_random_partial_withdrawals_test(spec, state, rng):
    ...
    for index in partially_withdrawable_indices:
        set_validator_partially_withdrawable(
            spec, state, index, excess_balance=rng.randint(1, 1000000000)
        )
