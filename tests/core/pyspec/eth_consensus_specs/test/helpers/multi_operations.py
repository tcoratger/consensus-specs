...
def _eligible_for_exit(spec, state, index):
    ...
    if is_post_electra(spec):
        no_pending_withdrawals = spec.get_pending_balance_to_withdraw(state, index) == spec.Gwei(0)
...
def get_random_deposit_requests(spec, state, rng, num_deposits=None):
    if num_deposits is None:
        if is_post_gloas(spec):
            ...
            num_deposits = rng.randint(0, 40_000)
        else:
    ...
    for _ in range(num_deposits):
        ...
        deposit, _, _ = build_deposit(
            ...
            privkeys[index],
            rng.randint(
                spec.EFFECTIVE_BALANCE_INCREMENT, spec.Gwei(2) * spec.MAX_EFFECTIVE_BALANCE_ELECTRA
            ),
            withdrawal_credentials=withdrawal_credentials,
            signed=True,
        )
